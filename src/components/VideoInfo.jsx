...
const VideoInfo = () => {
  ...
  const getInfos = async () => {
    ...
    const detailRes = await getData(
        `/video/info?id=${id}`)
       
    //önceki istektekten aldığımız kanal id'sini kullanarak
    // kanalın detay bilgilerine erişme
   const channelRes = await getData(
    `/channel/about?id=${detailRes.data.channelId}`)
     
   /*  console.log("video detay>>",detailRes.data);
    console.log("kanal detay>>",channelRes.data.meta);
 */
